...
### Frontend
Angular 17 with standalone components. HTTP calls use **axios** (not Angular's HttpClient). The `ApiService` (`src/app/services/api.service.ts`) is the single API layer. The base URL must match the backend port (8080).
